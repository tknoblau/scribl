...
Disk speed (IOPS) and the number of CPUs are generally your limiting factors.  Disk speed is a HUGE factor and usually pretty decent because it’s a general requirement for Splunk indexers.  You can monitor the linux processes to get a feel for whether they are in SLEEP or RUN mode.  If they spend the majority of their time in SLEEP mode, they are being throttled by disk, network, Cribl workers, etc and adding more CPUs will probably not buy you more speed.
